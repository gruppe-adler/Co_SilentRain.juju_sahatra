...
// close ramp 
{
	...
	[_jm, "jumpmaster_closingRamp", 65] remoteExec ["grad_drop_fnc_playSound",0,false];

	// _planeInBase engineOn true;
	sleep 1;
...
// start player sequence
{
	if (isPlayer _x && isNil "grad_drop_debug") then {
		[_x, _delayToTeleport] remoteExec ["grad_drop_fnc_moveInPlaneCut"];
	};
} forEach playableUnits + switchableUnits;

// setOvercast for everyone and force on server
[
	{
        // setting overcast seperate, to avoid race-conditions
        0 setOvercast 0.88;
        [0, 0.88] remoteExec ["setOvercast", -2];
		forceWeatherChange;
	},
	[],
	_delayToTeleport + 2
