...
// iterate players and move everyone into the corresponding plane in the sky
{
    ...
    if (_isInPlane) then {
            ...
            [{
                params ["_triggerPlane", "_planeInSky", "_unit", "_giveParachute", "_delay"];

                // for debug purposes
                if (_unit isKindOf "Man") then {
                    [_unit, _planeInSky, _giveParachute, _delay+1] remoteExec ["grad_drop_fnc_moveInRemote", _unit];
                };

            }, [_triggerPlane, _planeInSky, _unit, _giveParachute, _delay], _delay+1] call CBA_fnc_waitAndExecute;
        };
...
}, [_attachHelper, _planeInSky, _planeInBase], _delay] call CBA_fnc_waitAndExecute;
